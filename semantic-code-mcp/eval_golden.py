...
def main() -> None:
    ...
    for item in GOLDEN:
        ...
        rank = next((i for i, f in enumerate(files, 1) if f in expected), 0)
        top1 = 1 if (files and files[0] in expected) else 0
        recall5 = 1 if 0 < rank <= 5 else 0
        usable = 1 if (recall5 or (must and any(m in top5_code for m in must))) else 0
        rows.append({"cat": item["cat"], "query": q, "expected": sorted(expected),
                     "returned": files[:5], "rank": rank, "top1": top1,
                     "recall5": recall5, "usable": usable, "latency": round(elapsed, 2)})
        mark = f"rank={rank}" if rank else ("USABLE" if usable else "MISS")
    ...
    if "--json" in sys.argv:
        ...
        out.write_text(json.dumps({"engine": "semantic-code-mcp", "total": total,
                                   "by_category": by_cat, "rows": rows}, ensure_ascii=False, indent=2),
                       encoding="utf-8")
        print(f"\n[eval] JSON 已写入 {out}")
    store.close()
